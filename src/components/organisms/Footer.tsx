  ...
  return (
    <footer className="bg-foreground px-4 pt-16 lg:px-8 lg:pt-20">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="grid gap-10 sm:grid-cols-2 lg:grid-cols-5">
          ...
          {navColumns.map(({ title, links }) => (
            <div key={title}>
              ...
              <ul className="mt-4">
                {links.map((link) => (
                  <li key={link.label} className="flex h-9 items-center">
                    <a
                      href={link.href}
        ...
        {/* Bottom bar */}
        <div className="text-primary-foreground/40 md:text-md flex flex-col justify-between pb-10 text-sm sm:flex-row">
          <span>
