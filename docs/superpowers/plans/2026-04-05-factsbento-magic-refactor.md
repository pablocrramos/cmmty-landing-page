# FactsBento MagicBento Refactor Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Refactor `FactsBento.tsx` to add React Bits MagicBento animations (particles, spotlight, border glow, click ripple) adapted to the project's white/light-gray palette with a red glow color.

**Architecture:** All animation logic (ParticleCard, GlobalSpotlight) lives inline in `FactsBento.tsx` as private components — no new files. The component becomes a client component. Card content, layout, and Section/Container wrappers are unchanged.

**Tech Stack:** Next.js 16 (App Router), React 19, TypeScript strict, Tailwind CSS v4, GSAP
...
| `src/components/sections/FactsBento.tsx` | Full rewrite: add `"use client"`, GSAP imports, ParticleCard + GlobalSpotlight internals, adapted card rendering |
...
Expected output: `"gsap": "^3.x.x"` (some 3.x version)

- [ ] **Step 3: Commit**

```bash
git add package.json package-lock.json
git commit -m "chore: add gsap dependency for FactsBento animations"
```

---

### Task 2: Rewrite FactsBento.tsx

**Files:**

- Modify: `src/components/sections/FactsBento.tsx`

- [ ] **Step 1: Replace the entire file with the adapted implementation**

Replace `src/components/sections/FactsBento.tsx` with:
...
Expected: Build completes with no errors. Warnings about `mix-blend-mode` or dynamic class names are acceptable.
...
- Cards render with white background, dark text, gray border — same layout as before
- Hovering a card spawns small red particle dots that float around
- Mouse movement near/across cards shows a subtle red border glow tracing the mouse position
- Clicking a card shows a red ripple emanating from click point
- Red spotlight follows mouse across the bento grid section
- On mobile viewport (resize to <768px) — no animations, cards render statically

- [ ] **Step 6: Commit**

```bash
git add src/components/sections/FactsBento.tsx
git commit -m "feat: refactor FactsBento with MagicBento animations (particles, spotlight, border glow, ripple)"
```
